...
public class Handgun : MonoBehaviour, UseableObject
{
    ...
    void Update()
    {
        ...
        if(shotDelay >= 0.0f)
        {
            ...
            shotDelay = Mathf.Clamp(shotDelay, 0.0f, originDelay); // 발사딜레이 설정
        }
    }
    public void objectInteractive()
    {
        // 기본 소지 무기이므로 획득 상호작용 없음
    }
    ...
    public void shotHandgun()
    {
        ...
        if (shotDelay <= 0.0f && !Reloading) 
        {
            ...
            if (!noBullet) 
            {
                ...
                //ray에 저장된것을 토대로 생성하여 1000사거리 내의 Monster 레이어를 가진 대상이 ray에 닿으면 true반환
    ...
    {
        // 장전중이 아니고 장전된 총알수가 최대장전수와 같지 않을때
        if (!Reloading && (HaveBullet > 0) && (ReloadedBulletCount != canReloadBulletCount)) 
        {
    ...
    {        
        ...
        // 장전에 필요한 총알이 가진 총알보다 많을 때
        if (needBulletToReload > HaveBullet)                                  
        {
